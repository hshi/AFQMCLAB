...
void AfqmcMetropolis::initialField(WalkerLeft &walkerLeft, WalkerRight &walkerRight)
{
    ...
    applyOneBodyToLeftWalker(walkerLeft, walkerLeftInBlock[0], expMinusHalfDtK);
    logWeightLeftInBlock[0] = 0.0;

    currentTimeslice--;
    applyTwoBodySampleToRightWalker(walkerRightInBlock[method.timesliceBlockSize-1], walkerRight, twoBodySample);
    WalkerWalkerOperation walkerWalkerOperation(walkerLeftInBlock[0], walkerRight);
