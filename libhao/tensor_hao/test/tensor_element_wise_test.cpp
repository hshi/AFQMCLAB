...
TEST(Tensor_element, conj)
{
    ...
    for(HAO_INT j=0; j<2; j++)
    {
        for(HAO_INT i=0; i<3; i++)
        {
...
TEST(Tensor_element, add_Tensor)
{
    ...
    HAO_INT L = tensor_a.size();
    double* p_a = tensor_a.data();
    for(HAO_INT i=0; i<L; i++) p_a[i] = i*2.0;
...
TEST(Tensor_element, minus_Tensor)
{
    ...
    vector<double> exact(L);
    for (HAO_INT i = 0; i < L; ++i) exact[i] =  i;
...
TEST(Tensor_element, time_Tensor)
{
    ...
    vector<double> exact(L);
    for (HAO_INT i = 0; i < L; ++i) exact[i] =  i*i*2.0;
...
TEST(Tensor_element, divide_Tensor)
{
    ...
    vector<double> exact(L);
    for (HAO_INT i = 0; i < L; ++i) exact[i] =  2.0;
