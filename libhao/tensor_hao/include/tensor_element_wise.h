...
namespace tensor_hao
{
 ...
 TensorHao<std::complex<T>, D> conj(const TensorCore<std::complex<T>, D>& A)
 {
     ...
     for(HAO_INT i=0; i<L; i++) B_p[i] = std::conj( A_p[i] );
     return B;
 }
 ...
 TensorHao<T, D> exp(const TensorCore<T, D>& A)
 {
     ...
     const T * A_p = A.data();
           T * B_p = B.data();
     for(HAO_INT i=0; i<L; i++) B_p[i] = std::exp( A_p[i] );
     return B;
 }
 ...
 TensorHao<T, D> norm(const TensorCore<T, D>& A)
 {
     ...
     const T * A_p = A.data();
           T * B_p = B.data();
     for(HAO_INT i=0; i<L; i++) B_p[i] = std::norm( A_p[i] );
     return B;
 }
 ...
 TensorHao<T, D> abs(const TensorCore<T, D>& A)
 {
     ...
     const T * A_p = A.data();
     T * B_p = B.data();
     for(HAO_INT i=0; i<L; i++) B_p[i] = std::abs( A_p[i] );
     return B;
 }
 ...
 TensorHao<T, D> sqrt(const TensorCore<T, D>& A)
 {
     ...
     const T * A_p = A.data();
           T * B_p = B.data();
     for(HAO_INT i=0; i<L; i++) B_p[i] = std::sqrt( A_p[i] );
     return B;
 }
 ...
 TensorHao<T, D> operator - (const TensorCore<T, D>& A)
 {
     ...
     const T * A_p = A.data();
           T * B_p = B.data();
     for(HAO_INT i=0; i<L; i++) B_p[i] = -A_p[i];
     return B;
 }
 ...
 template <class T, HAO_INT D>
 HAO_INT diff(const TensorCore<T,D>& A, const TensorCore<T,D>& B, double eta)
 {
    ...
    //The result is 0 and 0.123
     ...
     HAO_INT flag=0; double abs_eta=std::abs(eta);

    for(HAO_INT i=0; i<D; i++) { if(A.rank(i) != B.rank(i) ) flag++; }
    for(HAO_INT i=0; i<D; i++) { if(A.rankStep(i) != B.rankStep(i) ) flag++; }
    if(A.size() != B.size() ) flag++;
    const T* A_p = A.data(); const T* B_p = B.data();
    for(HAO_INT i=0; i<A.size(); i++) { if( std::abs( A_p[i]- B_p[i] )> abs_eta  ) flag++; }
    return flag;
 }
 ...
 std::ostream& operator<< (std::ostream &out, const TensorCore<T,D>& object)
 {
     ...
     HAO_INT L = object.size(); const T* object_p = object.data();
     for (HAO_INT i = 0; i<L; ++i) out<<object_p[i]<<" ";
     return out;
 }
